...
async def _do_data_processing_task(data_processing_task):
    ...
    result_file_token = await CloudDFSConnector().put_data_file_async(job.result_filename, result_file_data)
...
async def _do_tensorflow_train_task(tensorflow_train_task):
    ...
    session_file_token = await CloudDFSConnector().put_data_file_async(job.session_filename, session_file_data)
